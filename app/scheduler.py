...
def setup_scheduler(app):
    ...
    job = scheduler.add_job(run_fetch, "cron", hour=hour, minute=minute, id="daily_fetch", misfire_grace_time=7200)
    if not auto_enabled:
        job.pause()
...
def reschedule(app, fetch_time: str, enabled: bool) -> None:
    ...
    hour, minute = map(int, fetch_time.split(":"))
    scheduler.reschedule_job("daily_fetch", trigger="cron", hour=hour, minute=minute, misfire_grace_time=7200)
    if enabled:
